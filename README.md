...
# FGCweb3.js - Web3.js for FantasyGold

FGCweb3 is a library for dApps to interract with the FantasyGold blockchain. FGCweb3 communicates to a FantasyGold node via the provider provided.

https://www.npmjs.com/package/fgcweb3 (Coming Soon)

## Get Started
Run the following in your project folder:

	npm install fgcweb3 --save

## Web Dapp Usage
This is example is meant for web dapps who would like to use FGCweb3's convenience methods with FGCLite's RPC provider. FGCLite is a FantasyGold wallet [Chrome extension]. More details about FGCLite [here](https://github.com/fantasygold/fgclite).

### 1. Construct FGCweb3 instance
If you have FGCLite installed, you will have a `window.fgclite` object injected in your browser tab. Pass that into FGCweb3 as a parameter to set the provider.
```
const fgcweb3 = new FGCweb3(window.fgclite.rpcProvider);
```
...
// Create a new Contract instance and use the same provider as fgcweb3
const contract = fgcweb3.Contract(contractAddress, contractAbi);
```

### 3. Get Logged In FGCLite Account
To get the current logged in account in FGCLite, you will have to add an [Event Listener](https://www.w3schools.com/jsref/met_element_addeventlistener.asp) to listen to messages sent from FGCLite.
```
let account;

function onFGCLiteAcctChange(event) {
  if (event.data.message && event.data.message.type == "ACCOUNT_CHANGED") {
    ...
    // You may also get the account from `window.fgclite.account`.
    // account = window.fgclite.account
  }
}
window.addEventListener('message', onFGCLiteAcctChange, false);
```

### 4. Execute sendtocontract
The last piece is to execute a `sendtocontract` on your Contract instance. This will automatically show a FGCLite popup to confirm that you would like to send the transaction.
```
...
## Fweb3Provider
The provider is the link between FGCweb3 and the blockchain. A compatible FGCweb3 Provider adheres to the following interface:
```
interface Fweb3Provider: {
  rawCall: (method: string, args: any[]) => Promise; // returns the result of the request
}
```

## FGCweb3
Instantiate a new instance of `FGCweb3`: 
```
const { FGCweb3 } = require('fgcweb3');

// Instantiate FGCweb3 with HttpProvider
// Pass in the URL of your FantasyGold node RPC port with auth credentials.
// Default FantasyGold RPC ports: testnet=58804 mainnet=57810
const fgcweb3 = new FGCweb3('http://fantasygold:fantasygold@localhost:58804');

// Instantiate FGCweb3 with FGCLiteRPCProvider
// FGCLiteRPCProvider is a provider for the FGCLite Wallet Chrome Extension.
// Please note FGCLiteRPCProvider only allows the rawCall() method to be used.
// It is specifically used for `sendtocontract` and `callcontract` only.
const fgcweb3 = new FGCweb3(window.fgclite.rpcProvider);
```

### isConnected()
Checks if you are connected properly to the local fantasygold node.
```
async function isConnected() {
  return await fgcweb3.isConnected();
}
```

### getHexAddress(address)
Converts a FantasyGold address to hex format.
```
async function getHexAddress() {
  return await fgcweb3.getHexAddress('fKjn4fStBaAtwGiwueJf9qFxgpbAvf1xAy');
}
```

### fromHexAddress(hexAddress)
Converts a hex address to FantasyGold format.
```
async function fromHexAddress() {
  return await fgcweb3.fromHexAddress('17e7888aa7412a735f336d2f6d784caefabb6fa3');
}
```

### getBlockCount()
Gets the current block height of your local FantasyGold node.
```
async function getBlockCount() {
  return await fgcweb3.getBlockCount();
}
```
...
async function getTransaction(args) {
  ...
  return await fgcweb3.getTransactionReceipt(transactionId);
}
```
...
async function listUnspent() {
  return await fgcweb3.listUnspent();
}
```
...
async function(args) {
  ...
  return await fgcweb3.searchLogs(fromBlock, toBlock, addresses, topics, contractMetadata, true);
}
```
...
// Create a Contract instance from the FGCweb3 instance
const contract = fgcweb3.Contract(contractAddress, contractAbi);
```
...
## Encoder
`Encoder` static functions are exposed in FGCweb3 instances.
```
const { FGCweb3 } = require('fgcweb3');

const fgcweb3 = new FGCweb3('http://fantasygold:fantasygold@localhost:58804');
fgcweb3.encoder.objToHash(abiObj, isFunction);
fgcweb3.encoder.addressToHex(address);
fgcweb3.encoder.boolToHex(value);
fgcweb3.encoder.intToHex(num);
fgcweb3.encoder.uintToHex(num);
fgcweb3.encoder.stringToHex(string, maxCharLen);
fgcweb3.encoder.stringArrayToHex(strArray, numOfItems);
fgcweb3.encoder.padHexString(hexStr);
fgcweb3.encoder.constructData(abi, methodName, args);
```

## Decoder
`Decoder` static functions are exposed in FGCweb3 instances.
```
const { FGCweb3 } = require('fgcweb3');

const fgcweb3 = new FGCweb3('http://fantasygold:fantasygold@localhost:58804');
fgcweb3.decoder.toFantasyGoldAddress(hexAddress, isMainnet);
fgcweb3.decoder.removeHexPrefix(value);
fgcweb3.decoder.decodeSearchLog(rawOutput, contractMetadata, removeHexPrefix);
fgcweb3.decoder.decodeCall(rawOutput, contractABI, methodName, removeHexPrefix);
```

## Utils
`Utils` static functions are exposed in FGCweb3 instances.
```
const { FGCweb3 } = require('fgcweb3');

const fgcweb3 = new FGCweb3('http://fantasygold:fantasygold@localhost:58804');
fgcweb3.utils.paramsCheck(methodName, params, required, validators);
fgcweb3.utils.appendHexPrefix(value);
fgcweb3.utils.trimHexPrefix(str);
fgcweb3.utils.chunkString(str, length);
fgcweb3.utils.toUtf8(hex);
fgcweb3.utils.fromUtf8(str);
fgcweb3.utils.isJson(str);
fgcweb3.utils.isFantasyGoldAddress(address);
```
...
```
FGC_RPC_ADDRESS='http://fantasygold:fantasygold@localhost:58804'
SENDER_ADDRESS='fMZK8FNPRm54jvTLAGEs1biTCgyCkcsmna'
WALLET_PASSPHRASE='fantasygold'
```
