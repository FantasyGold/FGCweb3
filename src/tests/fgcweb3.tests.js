...
const fAssert = require('../../test/utils/fassert');
const { getFantasyGoldRPCAddress, getDefaultFantasyGoldAddress, getWalletPassphrase, isWalletEncrypted } = require('../../test/utils');

const { assert } = chai;

console.log(`Your FantasyGold RPC address is ${getFantasyGoldRPCAddress()}`);
console.log(`Your Default FantasyGold address is ${getDefaultFantasyGoldAddress()}`);

describe('FGCweb3', () => {
  const FGC_ADDRESS = getDefaultFantasyGoldAddress();
  let fgcweb3;

  beforeEach(() => {
    fgcweb3 = new FGCweb3(getFantasyGoldRPCAddress());
  });

  /** ******** MISC ********* */
  describe('isConnected()', () => {
    it('returns true when connected', async () => {
      assert.isTrue(await fgcweb3.isConnected());
    });
  });
  ...
  describe('getBlockchainInfo()', () => {
    it('returns the block info', async () => {
      const res = await fgcweb3.getBlockchainInfo();
      assert.isDefined(res);
      assert.isObject(res);
  ...
  describe('getBlockCount()', () => {
    it('returns the blockcount', async () => {
      const res = await fgcweb3.getBlockCount();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('getBlockHash()', () => {
    it('returns the block hash', async () => {
      const res = await fgcweb3.getBlockHash(0);
      assert.isDefined(res);
      assert.isString(res);
    });
  });
  ...
  describe('listContracts()', () => {
    it('returns the array of deployed contracts', async () => {
      const res = await fgcweb3.listContracts();
      assert.isDefined(res);
      assert.isObject(res);
    });
  });
  ...
  describe('searchLogs()', () => {
    ...
    it('throws if fromBlock is not a number', async () => {
      assert.throws(() => fgcweb3.searchLogs(
        'a', 50100, [],
  ...
  describe('getPeerInfo()', () => {
    it('returns the Node info', async () => {
      const res = await fgcweb3.getPeerInfo();
      assert.isDefined(res);
      assert.isArray(res);
  ...
  describe('getHexAddress()', () => {
    it('returns the hex address', async () => {
      const hexDecodedAddress = bs58.decode(FGC_ADDRESS).toString('hex');
      const hexadecimalAddress = await fgcweb3.getHexAddress(FGC_ADDRESS);
  ...
  describe('fromHexAddress()', () => {
    it('returns the fantasygold address', async () => {
      const fantasygoldAddress = await fgcweb3.fromHexAddress('17e7888aa7412a735f336d2f6d784caefabb6fa3');
      assert.isString(fantasygoldAddress);
      assert.lengthOf(fantasygoldAddress, 34);
    });
  });
  ...
  describe('validateAddress()', () => {
    it('returns an object validating the address', async () => {
      let res = await fgcweb3.validateAddress(FGC_ADDRESS);
      assert.isDefined(res);
      assert.isDefined(res.isvalid);

      res = await fgcweb3.validateAddress('helloworld');
      assert.isDefined(res);
      assert.isDefined(res.isvalid);
    });
  });
  ...
  describe('dumpPrivateKey()', () => {
    it('returns the private key', async () => {
      const address = await fgcweb3.getAccountAddress('');
      if (await isWalletEncrypted(fgcweb3)) {
        await fgcweb3.walletPassphrase(getWalletPassphrase(), 3600);
        assert.isTrue((await fgcweb3.getWalletInfo()).unlocked_until > 0);

        const res = await fgcweb3.dumpPrivateKey(address);
        assert.isDefined(res);
        assert.isString(res);

        await fgcweb3.walletLock();
        assert.isTrue((await fgcweb3.getWalletInfo()).unlocked_until === 0);
      } else {
        const res = await fgcweb3.dumpPrivateKey(address);
        assert.isDefined(res);
  ...
  describe('getAccountAddress()', () => {
    it('returns the account address', async () => {
      const res = await fgcweb3.getAccountAddress('');
      assert.isDefined(res);
      assert.isString(res);
      assert.isTrue(res.startsWith('f') || res.startsWith('F'));
    });
  });

  describe('getAddressesByAccount()', () => {
    it('returns the account address array', async () => {
      const res = await fgcweb3.getAddressesByAccount('');
      assert.isDefined(res);
      assert.isArray(res);
      assert.isTrue(_.every(res, item => item.startsWith('f') || item.startsWith('F')));
    });
  });
    ...
    it('returns the transaction info', () => {
      const res = {
        ...
        details: [
          ...
          {
            account: '',
            address: 'fKjn4fStBaAtwGiwueJf9qFxgpbAvf1xAy',
            category: 'send',
            amount: -0.23241276,
            label: 'main',
            vout: 1,
  ...
  describe('getUnconfirmedBalance()', () => {
    it('returns the unconfirmed balance', async () => {
      const res = await fgcweb3.getUnconfirmedBalance();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('listAddressGroupings()', () => {
    it('returns an array of address groupings', async () => {
      const res = await fgcweb3.listAddressGroupings();
      assert.isDefined(res);
      assert.isArray(res);
      ...
      if (!_.isEmpty(res)) {
        ...
        if (!_.isEmpty(innerArr)) {
          ...
          fAssert.isFantasyGoldAddress(item[0]);
  ...
  describe('listLockUnspent()', () => {
    it('returns an array of unspendable outputs', async () => {
      const res = await fgcweb3.listLockUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('listUnspent()', () => {
    it('returns an unspent output array', async () => {
      const res = await fgcweb3.listUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('walletLock()', () => {
    it('locks the encrypted wallet', async () => {
      if (await isWalletEncrypted(fgcweb3)) {
        await fgcweb3.walletPassphrase(getWalletPassphrase(), 3600, true);
        assert.isTrue((await fgcweb3.getWalletInfo()).unlocked_until > 0);
  ...
  !_.includes(process.argv, '--cleanenv') ? describe.skip : describe('cleanEnv tests', () => {
    describe('getNewAddress()', () => {
      it('returns a new fantasygold address', async () => {
        const res = await fgcweb3.getNewAddress('');
        assert.isDefined(res);
        assert.isString(res);
        assert.isTrue(res.startsWith('f') || res.startsWith('F'));
      });
    ...
    describe('backupWallet()', () => {
      it('backup the wallet', async () => {
        const res = await fgcweb3.backupWallet(path.join(__dirname, '../../test/data/backup.dat'));
    ...
    describe('importWallet()', () => {
      it('throw an error if importing a non-existent file', async () => {
        try {
          await fgcweb3.importWallet(path.join(__dirname, '../../test/data/backup.dat'));
        } catch (err) {
      ...
      it('import the wallet from a wallet dump file', async () => {
        const res = await fgcweb3.importWallet(path.join(__dirname, '../../test/data/backup.dat'));
        assert.notTypeOf(res, 'Error');
      });
    });
  });
