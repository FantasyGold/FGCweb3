...
const MainnetNetworkByte = '23';
const TestnetNetworkByte = '5f';

class Decoder {
  static toFantasyGoldAddress(hexAddress, isMainnet = false) {
    ...
    let fAddress = hexAddress;
    // Add network byte
    if (isMainnet) {
      fAddress = MainnetNetworkByte + fAddress;
    } else {
      fAddress = TestnetNetworkByte + fAddress;
    }

    const fAddressBuffer = Buffer.from(fAddress, 'hex');
    // Double SHA256 hash
    const hash1 = crypto.createHash('sha256').update(fAddressBuffer).digest('Hex');
    const hash1Buffer = Buffer.from(hash1, 'hex');
    const hash2 = crypto.createHash('sha256').update(hash1Buffer).digest('Hex');

    // get first 4 bytes
    fAddress += hash2.slice(0, 8);

    // base58 encode
    const address = bs58.encode(Buffer.from(fAddress, 'hex'));
    return address;
  }
